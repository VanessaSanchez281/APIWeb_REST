//Para indicar la direccion de la URL donde esta la Web API
//NOTA:El numero de puerto es randomico y por lo tanto diferente
//para cada compuador, por lo tanto, es necesario adaptarlo a 
//cada computadora.Para esto ejecute la aplicacion y copi la url.

var api = "http://localhost:58881/api/categories/";

$(document).ready(function () {
    //Si la página de carga mostrará los registros en la etiqueta "Table"
    mostrarCategorias();
    $(document.body).on('click', 'a', function (e) {
        e.preventDefault();
        // Al hacer un click sobre la clave primaria de un registro de la tabla, buscar
...
function mostrarCategorias() {
    ...
    $.getJSON(api, function (datos) {
        ...
        $.each(datos, function (clave, valor) {
            ...
            // una fila con los datos, con lo cual se agrega celdas y filas a la etiqueta Table.
...
function buscarCategoria(href) {
    ...
    $.getJSON(href, function (datos) {
        ...
        // Desahibilitar (Bloquear) el TextBox que tiene la clave primaria, para
        // que se pueda ingresar datos en este TextBox.
        $('#id').attr("disabled", "disabled");
        // Poner los datos en el TextBox
...
function insertarCategoria() {
    ...
    $.ajax({
        ...
        error: mostrarError // Llamar el metodo
    });
};

// Usar AJAX para llamar el método con el prefijo "Put" de la Web API.
function actualizarCategoria() {
    $('#message').text("Actualizar...");
    $.ajax({
        type: 'PUT', // Metodo con el prefijo "Post" de la web API.
...
// Usar AJAX para llamar el método con el prefijo "Delete" de la Web API.
function eliminarCategoria() {
    $('#message').text("Eliminando...");
    $.ajax({
        type: 'DELETE', // Metodo con el prefijo "Post" de la web API.
        url: api + $('#id').val(), // Direccion de la URL mas la clave primaria
        data: $('#form1').serialize(),
        dataType: "json", // Formato JSon
        success: mostrarCategorias, // Llamar al metodo
...
// Mostrar el mensaje de error si no se puede conectar con el servidor
function mostrarError(solicitud, estado, error) {
    try {
        //Capturar el error si no se puede conectar con el servidor.
